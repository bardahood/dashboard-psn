<x-app-layout>
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-4">
                @can('profil.manage')
                    ...
                    <a href="{{ route('admin.analisis-rkp2027') }}" class="text-sm text-blue-700 font-medium hover:text-blue-900 hover:underline underline-offset-2 transition-colors">
                        Analisis Carryover RKP 2027 &rarr;
                    </a>
                @endcan
                @can('create', App\Models\Psn::class)
                    <a href="{{ route('admin.psn.create') }}" class="rounded-lg bg-blue-800 text-white shadow-sm hover:shadow transition-all px-4 py-2 text-sm hover:bg-blue-700">+ Tambah PSN</a>
                @endcan
            </div>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <form method="GET" class="bg-white shadow-sm ring-1 ring-gray-950/5 rounded-xl p-4 grid grid-cols-1 sm:grid-cols-4 gap-3">
                <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari nama PSN..."
                       class="col-span-1 sm:col-span-2 rounded-lg border-gray-300 transition-colors text-sm">
                ...
                <select name="kategori_usulan" class="rounded-lg border-gray-300 transition-colors text-sm">
                ...
                <button class="rounded-lg bg-gray-700 text-white shadow-sm transition-all text-sm font-medium py-2 hover:bg-gray-600">Filter</button>
            </form>
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3">
                                    @if ($psn->kategori_usulan)
                                        <span class="rounded-full text-xs px-2.5 py-1 font-medium {{ $psn->kategori_usulan === 'Carryover' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-700' }}">
                                            {{ $psn->kategori_usulan }}
                                        </span>
                                    @else
                                ...
                                <td class="px-4 py-3">
                                    <span class="rounded-full text-xs px-2.5 py-1 font-medium {{ $psn->sumber_input === 'Manual' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700' }}">
                                        {{ $psn->sumber_input }}
                                    </span>
                                </td>
