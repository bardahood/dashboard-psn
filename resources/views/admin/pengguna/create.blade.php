<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Tambah Pengguna</h2>
    </x-slot>

    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('admin.pengguna.store') }}" class="bg-white shadow rounded-lg p-6">
                @csrf
                @include('admin.pengguna._form')

                <div class="mt-6 flex justify-end gap-3">
                    <a href="{{ route('admin.pengguna.index') }}" class="rounded-md border px-4 py-2 text-sm">Batal</a>
                    <button class="rounded-md bg-blue-800 text-white px-4 py-2 text-sm hover:bg-blue-700">Simpan</button>
                </div>
            </form>
        </div>
    </div>
</x-app-layout>
