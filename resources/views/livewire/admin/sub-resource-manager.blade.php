            ...
            @foreach ($config['fields'] as $field)
                <div class="{{ in_array($field['type'], ['textarea']) ? 'sm:col-span-2' : '' }}">
                    ...
                    @elseif ($field['type'] === 'select')
                        <select wire:model="form.{{ $field['name'] }}" class="block w-full rounded-md border-gray-300 shadow-sm text-sm">
                    ...
                    @elseif ($field['type'] === 'number')
                        <input type="number" wire:model="form.{{ $field['name'] }}" class="block w-full rounded-md border-gray-300 shadow-sm text-sm">
                    @elseif ($field['type'] === 'date')
                        <input type="date" wire:model="form.{{ $field['name'] }}" class="block w-full rounded-md border-gray-300 shadow-sm text-sm">
                    @else
                        <input type="text" wire:model="form.{{ $field['name'] }}" class="block w-full rounded-md border-gray-300 shadow-sm text-sm">
                    @endif
                ...
                @forelse ($rows as $row)
                    <tr wire:key="row-{{ $row->id }}" class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 text-right space-x-3 whitespace-nowrap">
                            <button wire:click="edit({{ $row->id }})" class="text-blue-800 hover:underline">Ubah</button>
