<div class="space-y-6">
    <div class="bg-white shadow-sm ring-1 ring-gray-950/5 rounded-xl p-6">
        ...
        <form wire:submit="addDokumentasi" class="grid grid-cols-1 sm:grid-cols-4 gap-3">
            ...
            <div>
                ...
                <select wire:model="dokumentasiForm.kategori" class="block w-full rounded-lg border-gray-300 transition-colors shadow-sm text-sm">
            ...
            <div>
                ...
                <input type="text" wire:model="dokumentasiForm.deskripsi" class="block w-full rounded-lg border-gray-300 transition-colors shadow-sm text-sm">
            </div>
            <div class="flex items-end">
                <button type="submit" class="rounded-lg bg-blue-800 text-white shadow-sm hover:shadow transition-all px-4 py-2 text-sm hover:bg-blue-700 w-full">Unggah</button>
            </div>
        </form>
    </div>
            ...
            <tbody class="divide-y">
                @forelse ($dokumentasiList as $d)
                    <tr wire:key="dok-{{ $d->id }}">
                        ...
                        <td class="px-4 py-3">
                            <a href="{{ \Illuminate\Support\Facades\Storage::disk('public')->url($d->nama_file_tautan) }}" target="_blank" class="text-blue-700 font-medium hover:text-blue-900 hover:underline underline-offset-2 transition-colors">Lihat File</a>
                        </td>
    ...
    <div class="flex justify-between">
        ...
        <button wire:click="goToStep(9)" class="rounded-lg bg-blue-800 text-white shadow-sm hover:shadow transition-all px-4 py-2 text-sm hover:bg-blue-700">Lanjut ke Bagian I &rarr;</button>
    </div>
</div>
